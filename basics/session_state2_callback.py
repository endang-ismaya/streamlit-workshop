...
# store widget value with session state
...
def subtract_timedelta():
    initial = st.session_state["end_date"]
    if st.session_state["radio_range"] == "7 days":
        st.session_state["start_date"] = initial - timedelta(days=7)
    elif st.session_state["radio_range"] == "28 days":
        st.session_state["start_date"] = initial - timedelta(days=28)
    else:
        pass
